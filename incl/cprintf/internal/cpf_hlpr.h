...
#define CPF_MARK_CRITICAL_SECTION_ cpf::intern::user_thread_mutex.lock();

#define CPF_UNMARK_CRITICAL_SECTION_ cpf::intern::user_thread_mutex.unlock();
...
template <typename Op,
          ...
          template <std::size_t...> class I,
          std::size_t... Indices>
inline auto _apply_tuple(Op&& op, Tuple&& t, I<Indices...> &&) -> decltype(
...
inline auto apply_tuple(Op&& op, Tuple&& t) -> decltype(
  _apply_tuple(std::forward<Op>(op), std::forward<Tuple>(t), Indices{}))
{
...
void write_binary(
  cpf::type::stream_t ustream,
  typename std::enable_if<cpf::type::is_STL_string_type_<T>::value, T>::type&&
    arg)
{
}
...
cpf::type::rcode_t dispatch(FILE* stream, T0&& raw_format, Ts&&... args)
{
	...
	try
	{
		...
		cpf::intern::update_ustream(stream, mf_endpoint_cmp, mf_begin,
