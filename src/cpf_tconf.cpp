...
cpf::type::str_t get_terminal_bitmap_colour_value(
  const cpf::type::str_t& attrib_token)
{
                         ...
                         const cpf::type::str_t& attrib_str)
{
	...
	fwprintf(
	  user_stream, L"%S",
	  (L"\x1B[" + vertical_pos_str + L";" + horizontal_pos_str + L"H").c_str());
#endif
}
...
CPF_API void cpf::intern::configure(
  cpf::type::stream_t               user_stream,
	...
	if (crnt_txt_attribs != attribute_group)
	{
		     ...
		     ++a)
		{
			...
			else /*set colour attribute(s)*/
			{
				...
				const cpf::type::colour colour_value = get_token_value(tok);
