...
{
	cpf::intern::configure(file_stream, attr);
		...
		for (const auto& xfst : cpf::intern::ext_fmtspec_terms)
		{
			...
			for (auto i(0u); i < _max; ++i)
			{
				...
				if (isalnum(crnt_char) ||
				    is_in(crnt_char, cpf::intern::inter_fmt_specs) || crnt_char == xfst)
				{
...
void cpf::intern::write_non_arg_str(
  ...
  cpf::type::meta_fmt_t::const_iterator& meta_iter)
{
	cpf::intern::configure(file_stream, meta_iter->second.first);
...
#pragma GCC diagnostic pop
	...
	/*restore defaults*/
	cpf::intern::configure(file_stream, cpf::type::str_vec_t({ L"?" }));
}
